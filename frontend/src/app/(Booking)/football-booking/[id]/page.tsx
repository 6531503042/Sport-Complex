...
function Football_Booking({ params }: UserDataParams) {
  ...
  const getUserData = async (id: string) => {
    try {
      const resUserData = await fetch(`http://localhost:1325/user_v1/users/${id}`, {
        method: "GET",
        cache: "no-store",
      });
      if (!resUserData.ok) {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {slot.map((lot) => (
                 ...
                 className={`bg-white border border-gray-200 rounded-lg p-6 shadow-md transition-transform duration-300 ease-in-out
                   ${lot.current_bookings ? "cursor-not-allowed" : "cursor-pointer hover:scale-105 hover:shadow-lg"}
                   ${!lot.current_bookings ? "" : "bg-[#E4E0E1]"}
                   ${!lot.current_bookings ? "hover:bg-green-200" : "bg-gray text-gray-700"}
                 `}
                 onClick={() => {
                   if (!lot.current_bookings) {
